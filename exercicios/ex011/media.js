...
function media () {
  const nom = window.prompt('Qual é o nome do aluno?')
  const n1 = Number(window.prompt(`Qual foi a primeira nota de ${nom}?`))
  const n2 = Number(window.prompt(`Além de ${n1}, qual foi a outra nota de ${nom}?`))
  med = (n1 + n2) / 2
  ...
  const res = document.getElementById('situacao')
